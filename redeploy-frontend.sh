...
echo "[INFO] frontend image: ${FRONTEND_IMAGE:-<compose default>}"

echo "[STEP 1/4] stopping frontend container..."
"${compose_cmd[@]}" -f "$COMPOSE_FILE" stop "$SERVICE_NAME" || true

echo "[STEP 2/4] pulling latest frontend image from Docker Hub..."
...
echo "[STEP 4/4] current frontend container status:"
